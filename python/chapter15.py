...
# Im1 = pyglet.image.load('./image/BlueTri.jpg')
Im1 = pyglet.image.load('./image/BlueTri.png')
Im2 = pyglet.image.load('./image/PurpleStar.png')
Im3 = pyglet.image.load('./image/OrangeDiamond.png')
Im4 = pyglet.image.load('./image/YellowCircle.png')
Im5 = pyglet.image.load('./image/RedHex.png')
...
def on_draw():
    ...
    label = pyglet.text.Label('Turn: '+str(turn)+' Score:'+str(score),
                              font_name='Arial', font_size=18, x=20,y = 10)
    label.draw()
...
def on_mouse_release(x, y, button, modifiers):
    ...
    endcol = x//50
    endrow = (y-50)//50
    print("start, end: {}-{}, {}-{}".format(startrow, startcol, endrow, endcol))
    # make move.
    if ((startcol==endcol and startrow==endrow - 1) or
        (startcol==endcol and startrow==endrow+1) or
